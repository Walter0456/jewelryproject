...
-- Complete Schema for JewelAdmin Pro
-- Last Updated: 2024
...
-- Ensure users table has profile_picture column
DO $$ 
BEGIN 
...
-- Ensure settings table has branches column
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='branches') THEN
        ALTER TABLE settings ADD COLUMN branches JSONB DEFAULT '[]'::jsonb;
    END IF;
END $$;

-- Ensure settings table has backup_settings column
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='backup_settings') THEN
        ALTER TABLE settings ADD COLUMN backup_settings JSONB DEFAULT '{}'::jsonb;
    END IF;
END $$;

-- Ensure settings table has receipt_layout column
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='receipt_layout') THEN
        ALTER TABLE settings ADD COLUMN receipt_layout JSONB DEFAULT '{}'::jsonb;
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS settings (
    ...
    sales_summary_alerts BOOLEAN DEFAULT true,
    security_alerts BOOLEAN DEFAULT true,
    inventory_sync_alerts BOOLEAN DEFAULT true,
    desktop_notifications BOOLEAN DEFAULT true,
    email_notifications BOOLEAN DEFAULT false,
    alert_email_address VARCHAR(255),
    daily_summary_time TIME DEFAULT '18:00',
    low_stock_check_frequency VARCHAR(20) DEFAULT 'hourly'
);
...
INSERT INTO settings (id, branch_name, currency_symbol, tax_rate, low_stock_threshold, time_format, receipt_auto_print, profit_margin, max_images_per_product, staff_can_edit_price, admin_pin, session_timeout_minutes, branches)
